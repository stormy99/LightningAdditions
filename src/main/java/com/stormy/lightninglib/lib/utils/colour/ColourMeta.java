...
public enum ColourMeta implements IStringSerializable {
    ...
    private ColourMeta(String name, int rgb, EnumDyeColor vanilla) {
        this(name, name, rgb, vanilla);
    }

    private ColourMeta(String name, String textureName, int rgb, EnumDyeColor vanilla) {
    ...
    static final ColourMeta[] VALUES = ColourMeta.values();

    private static final Map<String, ColourMeta> COLORS_BY_ORE_NAME = Maps.newHashMap();
    private static final Map<String, ColourMeta> COLORS_BY_NAME = Maps.newHashMap();
    private static final Map<Integer, ColourMeta> COLORS_BY_ORE_ID = Maps.newHashMap();
    private static final Map<Integer, ColourMeta> COLORS_BY_BITMASK = Maps.newHashMap();
    private static final Map<EnumDyeColor, ColourMeta> COLORS_BY_VANILLA_ENUM = Maps.newEnumMap(EnumDyeColor.class);

    static {
        for (ColourMeta color : values()) {
    ...
    public static Set<ColourMeta> fromStack(@Nonnull ItemStack stack) {
        Set<ColourMeta> result = Sets.newIdentityHashSet();
        for (int oreId : OreDictionary.getOreIDs(stack)) {
            ColourMeta meta = COLORS_BY_ORE_ID.get(oreId);
            if (meta != null) result.add(meta);
        }
        return result;
    }

    public static ColourMeta fromOreId(int oreId) {
        return COLORS_BY_ORE_ID.get(oreId);
    }

    public static ColourMeta fromOreName(String oreName) {
        return COLORS_BY_ORE_NAME.get(oreName);
    }

    public static ColourMeta fromName(String name) {
        return COLORS_BY_NAME.get(name.toLowerCase(Locale.ENGLISH));
    }

    public static ColourMeta fromBitmask(int bitmask) {
        return COLORS_BY_BITMASK.get(bitmask);
    }

    public static ColourMeta fromVanillaEnum(EnumDyeColor color) {
        return COLORS_BY_VANILLA_ENUM.get(color);
    }

    public static ColourMeta fromBlockMeta(int meta) {
        final EnumDyeColor vanilla = EnumDyeColor.byMetadata(meta);
        return fromVanillaEnum(vanilla);
    }

    public static ColourMeta fromDyeDamage(int damage) {
        final EnumDyeColor vanilla = EnumDyeColor.byDyeDamage(damage);
        return fromVanillaEnum(vanilla);
    }

    public static Collection<ColourMeta> getAllColors() {
